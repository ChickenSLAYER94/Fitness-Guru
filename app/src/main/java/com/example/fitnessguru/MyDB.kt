...
class MyDB(context: Context) : SQLiteOpenHelper(context, "USERDB", null, 1) {
    ...
    fun getDetailsAccordingToDate(): ArrayList<fitnessData> {
        ...
        //retriving data according to the date
        val cursor: Cursor? = db.rawQuery("SELECT * FROM STEPCOUNTER GROUP by Date", null)
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    RemainingSteplimit =
                        cursor.getString(cursor.getColumnIndex("RemainingSteplimit"))
